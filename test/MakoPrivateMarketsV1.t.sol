...
contract MakoPrivateMarketsV1Test is Test {
    ...
    function _friendlyParams(uint64 opensAt, uint64 closeAt) internal pure returns (MakoPrivateMarketsV1.CreateParams memory p) {
    ...
    function _openVoteParams(uint64 opensAt, uint64 closeAt, uint256 fixedStake_, uint8 winners) internal pure returns (MakoPrivateMarketsV1.CreateParams memory p) {
    ...
    function _prizePoolParams(uint64 opensAt, uint64 closeAt, address[] memory wallets, uint8 winners) internal pure returns (MakoPrivateMarketsV1.CreateParams memory p) {
    ...
    function _createFriendly() internal returns (uint256 id) {
        MakoPrivateMarketsV1.CreateParams memory p = _friendlyParams(uint64(block.timestamp), uint64(block.timestamp + 1 days));
        vm.prank(creator);
        id = pm.createMarket(p);
    }

    function _createOpenVote(uint256 fixedStake_, uint8 winners) internal returns (uint256 id) {
        MakoPrivateMarketsV1.CreateParams memory p = _openVoteParams(uint64(block.timestamp), uint64(block.timestamp + 1 days), fixedStake_, winners);
        vm.prank(creator);
        id = pm.createMarket(p);
    }

    function _createPrizePool(address[] memory wallets, uint8 winners) internal returns (uint256 id) {
        MakoPrivateMarketsV1.CreateParams memory p = _prizePoolParams(uint64(block.timestamp), uint64(block.timestamp + 1 days), wallets, winners);
        vm.prank(creator);
        id = pm.createMarket(p);
    }
    ...
    function test_lifecycle_immediateOpenAccepted() public {
        MakoPrivateMarketsV1.CreateParams memory p = _friendlyParams(uint64(block.timestamp), uint64(block.timestamp + 1 hours));
        vm.prank(creator);
    ...
    function test_lifecycle_futureOpenAccepted() public {
        MakoPrivateMarketsV1.CreateParams memory p = _friendlyParams(uint64(block.timestamp + 1 hours), uint64(block.timestamp + 2 hours));
        vm.prank(creator);
        uint256 id = pm.createMarket(p);
    ...
    function test_lifecycle_pastOpenReverts() public {
        MakoPrivateMarketsV1.CreateParams memory p = _friendlyParams(uint64(block.timestamp - 1), uint64(block.timestamp + 1 hours));
        vm.prank(creator);
    ...
    function test_lifecycle_closeBeforeOpenReverts() public {
        MakoPrivateMarketsV1.CreateParams memory p = _friendlyParams(uint64(block.timestamp + 100), uint64(block.timestamp + 50));
        vm.prank(creator);
        vm.expectRevert(MakoPrivateMarketsV1.InvalidTimestamps.selector);
        pm.createMarket(p);
    }
    ...
    function test_lifecycle_closeEqualOpenReverts() public {
        MakoPrivateMarketsV1.CreateParams memory p = _friendlyParams(uint64(block.timestamp + 100), uint64(block.timestamp + 100));
        vm.prank(creator);
        vm.expectRevert(MakoPrivateMarketsV1.InvalidTimestamps.selector);
        pm.createMarket(p);
    }
    ...
    function test_lifecycle_creatorActionAtCloseAcceptedWithStake() public {
        uint256 id = _createFriendly();
        vm.prank(alice); pm.bet(id, FRIENDLY_YES, 1e6);
        vm.prank(bob);   pm.bet(id, FRIENDLY_NO,  1e6);
        vm.warp(block.timestamp + 1 days); // == closeAt
        vm.prank(creator);
    ...
    function test_create_optionLabelTooLargeReverts() public {
        MakoPrivateMarketsV1.CreateParams memory p = _openVoteParams(uint64(block.timestamp), uint64(block.timestamp + 1 hours), 1e6, 2);
    ...
    function test_create_tooManyOptionsReverts() public {
        ...
        for (uint256 i = 0; i < 51; i++) p.optionLabels[i] = bytes("X");
        vm.prank(creator);
    ...
    function test_create_winnersZeroReverts() public {
        MakoPrivateMarketsV1.CreateParams memory p = _openVoteParams(uint64(block.timestamp), uint64(block.timestamp + 1 hours), 1e6, 0);
        vm.prank(creator);
    ...
    function test_create_winnersElevenReverts() public {
        MakoPrivateMarketsV1.CreateParams memory p = _openVoteParams(uint64(block.timestamp), uint64(block.timestamp + 1 hours), 1e6, 11);
        // Need >= 11 options for the winners-vs-options check to not trip first
        p.optionLabels = new bytes[](12);
        for (uint256 i = 0; i < 12; i++) p.optionLabels[i] = bytes("X");
        vm.prank(creator);
        vm.expectRevert(MakoPrivateMarketsV1.InvalidWinners.selector);
        pm.createMarket(p);
    }
    ...
    function test_create_winnersExceedOptionsReverts() public {
        MakoPrivateMarketsV1.CreateParams memory p = _openVoteParams(uint64(block.timestamp), uint64(block.timestamp + 1 hours), 1e6, 5);
        p.optionLabels = new bytes[](3);
        p.optionLabels[0] = bytes("a"); p.optionLabels[1] = bytes("b"); p.optionLabels[2] = bytes("c");
        vm.prank(creator);
        vm.expectRevert(MakoPrivateMarketsV1.InvalidWinners.selector);
        pm.createMarket(p);
    }
    ...
    function test_create_participantsDuplicatedReverts() public {
        ...
        wallets[0] = alice; wallets[1] = bob; wallets[2] = alice;
        MakoPrivateMarketsV1.CreateParams memory p = _prizePoolParams(uint64(block.timestamp), uint64(block.timestamp + 1 hours), wallets, 1);
        vm.prank(creator);
    ...
    function test_create_participantZeroReverts() public {
        ...
        wallets[0] = alice; wallets[1] = address(0);
        MakoPrivateMarketsV1.CreateParams memory p = _prizePoolParams(uint64(block.timestamp), uint64(block.timestamp + 1 hours), wallets, 1);
        vm.prank(creator);
        vm.expectRevert(MakoPrivateMarketsV1.InvalidParticipants.selector);
        pm.createMarket(p);
    ...
    function test_create_participantTreasuryReverts() public {
        address[] memory wallets = new address[](2);
        wallets[0] = alice; wallets[1] = treasury;
        MakoPrivateMarketsV1.CreateParams memory p = _prizePoolParams(uint64(block.timestamp), uint64(block.timestamp + 1 hours), wallets, 1);
        vm.prank(creator);
    ...
    function test_create_allowlistTreasuryReverts() public {
        ...
        p.allowlist[0] = alice; p.allowlist[1] = treasury;
        vm.prank(creator);
        vm.expectRevert(MakoPrivateMarketsV1.TreasuryNotAllowed.selector);
        pm.createMarket(p);
    }
    ...
    function test_create_allowlistTooLargeReverts() public {
        ...
        for (uint256 i = 0; i < 101; i++) p.allowlist[i] = address(uint160(i + 1));
        vm.prank(creator);
    ...
    function test_friendly_pariMutuelPayout() public {
        ...
        vm.prank(alice); pm.bet(id, FRIENDLY_YES, 100e6);
        vm.prank(bob);   pm.bet(id, FRIENDLY_NO,  200e6);
        vm.warp(block.timestamp + 1 days);
        vm.prank(creator); pm.resolve(id, FRIENDLY_YES);
    ...
    function test_friendly_multiBetAggregation() public {
        uint256 id = _createFriendly();
        vm.prank(alice); pm.bet(id, FRIENDLY_YES, 30e6);
        vm.prank(alice); pm.bet(id, FRIENDLY_YES, 70e6);
        vm.prank(bob);   pm.bet(id, FRIENDLY_NO,  200e6);
        vm.warp(block.timestamp + 1 days);
        vm.prank(creator); pm.resolve(id, FRIENDLY_YES);
    ...
    function test_friendly_emptyYesPoolResolveYes() public {
        uint256 id = _createFriendly();
        vm.prank(bob); pm.bet(id, FRIENDLY_NO, 50e6);
        vm.warp(block.timestamp + 1 days);
        vm.prank(creator); pm.resolve(id, FRIENDLY_YES);
    ...
    function test_friendly_emptyNoPoolResolveYes() public {
        uint256 id = _createFriendly();
        vm.prank(alice); pm.bet(id, FRIENDLY_YES, 50e6);
        vm.warp(block.timestamp + 1 days);
        vm.prank(creator); pm.resolve(id, FRIENDLY_YES);
    ...
    function test_friendly_timeoutRefundsAll() public {
        uint256 id = _createFriendly();
        vm.prank(alice); pm.bet(id, FRIENDLY_YES, 30e6);
        vm.prank(bob);   pm.bet(id, FRIENDLY_NO,  20e6);
        vm.warp(block.timestamp + 1 days + 7 days);
    ...
    function test_friendly_loserClaimReverts() public {
        uint256 id = _createFriendly();
        vm.prank(alice); pm.bet(id, FRIENDLY_YES, 100e6);
        vm.prank(bob);   pm.bet(id, FRIENDLY_NO, 100e6);
        vm.warp(block.timestamp + 1 days);
        vm.prank(creator); pm.resolve(id, FRIENDLY_YES);
        vm.prank(bob);
    ...
    function test_friendly_sameWalletHedge() public {
        uint256 id = _createFriendly();
        vm.prank(alice); pm.bet(id, FRIENDLY_YES, 10e6);
        vm.prank(alice); pm.bet(id, FRIENDLY_NO, 5e6);
        vm.prank(bob); pm.bet(id, FRIENDLY_NO, 95e6);
        vm.warp(block.timestamp + 1 days);
        vm.prank(creator); pm.resolve(id, FRIENDLY_YES);
    ...
    function test_openVote_oneStakePerWallet() public {
        uint256 id = _createOpenVote(2e6, 2);
        vm.prank(alice); pm.stake(id, 0, 2e6);
        vm.prank(alice);
    ...
    function test_openVote_confirmRanksAndRefunds() public {
        ...
        usdc.mint(voters[4], 100e6); usdc.mint(voters[5], 100e6);
        vm.prank(voters[4]); usdc.approve(address(pm), type(uint256).max);
        vm.prank(voters[5]); usdc.approve(address(pm), type(uint256).max);
        vm.prank(alice);  pm.stake(id, 0, 10e6);
        vm.prank(bob);    pm.stake(id, 0, 10e6);
        vm.prank(carol);  pm.stake(id, 0, 10e6);
        vm.prank(dave);   pm.stake(id, 1, 10e6);
        vm.prank(voters[4]); pm.stake(id, 1, 10e6);
        vm.prank(voters[5]); pm.stake(id, 2, 10e6);
        vm.warp(block.timestamp + 1 days);
        vm.prank(creator); pm.confirm(id);
    ...
    function test_openVote_cancelRefundsFull() public {
        ...
        vm.prank(alice); pm.stake(id, 0, 5e6);
        vm.prank(bob);   pm.stake(id, 1, 5e6);
        vm.warp(block.timestamp + 1 days);
        vm.prank(creator); pm.cancel(id);
    ...
    function test_prizePool_distributeProportional() public {
        ...
        wallets[0] = address(0xA1); wallets[1] = address(0xA2); wallets[2] = address(0xA3); wallets[3] = address(0xA4);
        uint256 id = _createPrizePool(wallets, 3);
        // Stakes: option 0 = 500, 1 = 300, 2 = 100, 3 = 50, total=950
        vm.prank(alice); pm.stake(id, 0, 500e6);
        vm.prank(bob);   pm.stake(id, 1, 300e6);
        vm.prank(carol); pm.stake(id, 2, 100e6);
        vm.prank(dave);  pm.stake(id, 3, 50e6);
        vm.warp(block.timestamp + 1 days);
        vm.prank(creator); pm.distribute(id);
    ...
    function test_prizePool_perWalletCumulativeCap() public {
        address[] memory wallets = new address[](2);
        wallets[0] = address(0xA1); wallets[1] = address(0xA2);
        MakoPrivateMarketsV1.CreateParams memory p = _prizePoolParams(uint64(block.timestamp), uint64(block.timestamp + 1 days), wallets, 1);
        p.perWalletCumulativeMax = 100e6;
        vm.prank(creator);
        uint256 id = pm.createMarket(p);
        vm.prank(alice); pm.stake(id, 0, 60e6);
        vm.prank(alice); pm.stake(id, 1, 40e6);
        vm.prank(alice);
    ...
    function test_prizePool_sameWalletParticipantAndStaker() public {
        ...
        vm.prank(alice); pm.stake(id, 0, 100e6); // alice stakes on her own option
        vm.prank(carol); pm.stake(id, 1, 50e6);
        vm.warp(block.timestamp + 1 days);
        vm.prank(creator); pm.distribute(id);
    ...
    function test_prizePool_cancelRefundsAll() public {
        ...
        wallets[0] = address(0xA1); wallets[1] = address(0xA2);
        uint256 id = _createPrizePool(wallets, 1);
        vm.prank(alice); pm.stake(id, 0, 30e6);
        vm.prank(alice); pm.stake(id, 1, 20e6);
        vm.warp(block.timestamp + 1 days);
        vm.prank(creator); pm.cancel(id);
        assertEq(pm.getPendingClaim(id, alice), 50e6);
    }
    ...
    function test_tieBreaker_lowerFirstStakeSequenceWins() public {
        ...
        vm.prank(alice); pm.stake(id, 1, 10e6); // option 1 first
        vm.prank(bob);   pm.stake(id, 0, 10e6); // option 0 second
        vm.warp(block.timestamp + 1 days);
        vm.prank(creator); pm.confirm(id);
        (uint256[] memory topN,,) = pm.getVoteResolution(id);
    ...
    function test_tieBreaker_capStrictlyTen() public {
        ...
        uint256 id = _createOpenVote(10e6, 2);
        vm.prank(alice); pm.stake(id, 0, 10e6);
        vm.prank(bob);   pm.stake(id, 1, 10e6);
        vm.prank(carol); pm.stake(id, 2, 10e6);
        vm.prank(dave);  pm.stake(id, 3, 10e6);
        vm.warp(block.timestamp + 1 days);
        vm.prank(creator); pm.confirm(id);
        (uint256[] memory topN,,) = pm.getVoteResolution(id);
        assertEq(topN.length, 2);
    ...
    function test_treasury_cannotBet() public {
        ...
        vm.prank(treasury); usdc.approve(address(pm), type(uint256).max);
        vm.prank(treasury);
        vm.expectRevert(MakoPrivateMarketsV1.TreasuryNotAllowed.selector);
        pm.bet(id, FRIENDLY_YES, 1e6);
    }
    ...
    function test_view_optionsAndAllowlist() public {
        ...
        p.participationMode = MakoPrivateMarketsV1.VisibilityParticipation.Allowlisted;
        p.allowlist = new address[](2);
        p.allowlist[0] = alice; p.allowlist[1] = bob;
        vm.prank(creator);
        uint256 id = pm.createMarket(p);
    ...
    function test_edge_claimLazyTimedOutBeforeFinalize() public {
        ...
        assertEq(uint8(pm.getMarket(id).effectiveState), STATE_TIMED_OUT);
        vm.prank(alice); pm.claim(id); // succeeds without finalize
    }
    ...
    function test_openVote_nonRoundFixedStake_noDustUnderflow() public {
        ...
        vm.prank(alice); pm.stake(id, 0, 10_099);
        vm.prank(bob);   pm.stake(id, 1, 10_099);
        vm.warp(block.timestamp + 1 days);
        vm.prank(creator); pm.confirm(id);
    ...
    function test_friendly_dustAccruesExactlyOneBaseUnit() public {
        uint256 id = _createFriendly();
        vm.prank(alice); pm.bet(id, FRIENDLY_YES, 10_000);
        vm.prank(bob);   pm.bet(id, FRIENDLY_YES, 10_000);
        vm.prank(carol); pm.bet(id, FRIENDLY_NO,  10_001);
        vm.warp(block.timestamp + 1 days);
        vm.prank(creator); pm.resolve(id, FRIENDLY_YES);
    ...
    function test_view_getOptionFirstStakeSequence() public {
        uint256 id = _createOpenVote(10e6, 1);
        vm.prank(alice); pm.stake(id, 2, 10e6); // option 2 first
        vm.prank(bob);   pm.stake(id, 0, 10e6); // option 0 second
    ...
    /// amount, timestamp).
    event Staked(uint256 indexed marketId, address indexed staker, uint256 optionIndex, uint256 amount, uint256 timestamp);
    ...
    event ResolvedFriendly(uint256 indexed marketId, uint8 outcome, bool emptyPoolPath, uint256 feeTaken, uint256 totalOwed);
    ...
    function test_event_distributedPrizePoolPayload() public {
        address[] memory wallets = new address[](3);
        wallets[0] = address(0xA1); wallets[1] = address(0xA2); wallets[2] = address(0xA3);
        uint256 id = _createPrizePool(wallets, 2);
        vm.prank(alice); pm.stake(id, 0, 200e6);
        vm.prank(bob);   pm.stake(id, 1, 100e6);
        vm.prank(carol); pm.stake(id, 2, 50e6);
        vm.warp(block.timestamp + 1 days);
    ...
    function test_claim_transferFailureRollsBackUserClaimed() public {
        ...
        vm.prank(alice); flaky.approve(address(fpm), type(uint256).max);
        vm.prank(bob);   flaky.approve(address(fpm), type(uint256).max);

        MakoPrivateMarketsV1.CreateParams memory p = _friendlyParams(uint64(block.timestamp), uint64(block.timestamp + 1 days));
        vm.prank(creator);
        uint256 id = fpm.createMarket(p);
        vm.prank(alice); fpm.bet(id, FRIENDLY_YES, 10e6);
        vm.prank(bob);   fpm.bet(id, FRIENDLY_NO, 10e6);
        vm.warp(block.timestamp + 1 days);
        vm.prank(creator); fpm.resolve(id, FRIENDLY_YES);
    ...
    function test_claim_transferFailureRollsBackTreasuryCounter() public {
        ...
        flaky.setTransferShouldRevert(false);
        vm.prank(treasury); fpm.claim(id);
    ...
    function test_stake_feeOnTransferReverts() public {
        ...
        vm.prank(alice); fot.approve(address(fpm), type(uint256).max);
    ...
    function test_openVoteStake_feeOnTransferReverts() public {
        ...
        MakoPrivateMarketsV1.CreateParams memory p = _openVoteParams(uint64(block.timestamp), uint64(block.timestamp + 1 days), 1e6, 2);
        vm.prank(creator);
        uint256 id = fpm.createMarket(p);
        vm.prank(alice);
        vm.expectRevert(MakoPrivateMarketsV1.TransferAmountMismatch.selector);
    ...
    function test_gas_prizePoolCancelClaim50Options() public {
        ...
        for (uint256 i = 0; i < 50; i++) wallets[i] = address(uint160(0x100 + i));
        uint256 id = _createPrizePool(wallets, 5);
        // alice stakes on every option
        for (uint256 i = 0; i < 50; i++) {
            vm.prank(alice); pm.stake(id, i, 1e6);
        }
        vm.warp(block.timestamp + 1 days);
        vm.prank(creator); pm.cancel(id);
        uint256 g0 = gasleft();
