# Deploy MakoPrivateMarketsV1 to Monad testnet.
#
...
#   .\deploy-private-markets.ps1
#
# Constructor signature: (usdc, treasury). Same treasury wallet as v4
# per locked plan. Same USDC address as v4 (canonical Monad testnet USDC).
...
Write-Host "Deploying MakoPrivateMarketsV1 to Monad testnet..." -ForegroundColor Cyan
Write-Host "  TREASURY     = $env:TREASURY"
Write-Host "  USDC_ADDRESS = $env:USDC_ADDRESS"
Write-Host ""

& node "../mako-markets/scripts/with-bw.mjs" forge script "script/DeployPrivateMarketsV1.s.sol:DeployPrivateMarketsV1" --rpc-url monad_testnet --broadcast --legacy
